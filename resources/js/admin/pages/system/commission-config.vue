<template>
    <div>
        <div class="mb-6">
            <h1 class="text-h4 font-weight-bold mb-1">Konfigurasi Komisi</h1>
            <p class="text-body-2 text-medium-emphasis">
                Halaman untuk mengelola konfigurasi komisi dan reward DNY
                Skincare.
            </p>
        </div>
...
import type { SystemConfig, ConfigEntry } from "@/admin/types/system-config";
import commissionConfigService from "@/admin/services/commission-config.service";
...
const formatEntryValue = (entry: ConfigEntry): string => {
    if (
        entry.key.includes("percentage") ||
        entry.key.includes("basis_points")
    ) {
        return entry.key.includes("basis_points")
            ? `${Number(entry.value) / 100}%`
            : `${entry.value}%`;
    }
    if (entry.key.includes("amount")) {
        return `Rp${formatPrice(Number(entry.value))}`;
    }
...
const fetchConfigs = async () => {
    ...
    try {
        const response = await commissionConfigService.getConfigs({
            sort: "id",
            limit: 50,
        });
        configs.value = response.results;
    } catch (error: any) {
        snackbar.showMessage("Gagal memuat konfigurasi komisi", "error");
    } finally {
        isLoading.value = false;
    }
};

const handleSaveConfig = async (id: number, values: Record<string, any>) => {
    await commissionConfigService.updateConfig(id, {
        value: values,
    });
};
