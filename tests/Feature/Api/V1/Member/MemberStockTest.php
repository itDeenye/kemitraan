...
class MemberStockTest extends TestCase
{
    ...
    public function test_member_stock_requires_an_authenticated_member(): void
    {
        $this->getJson('/api/v1/member/inventory/stocks')->assertUnauthorized();
    }

    public function test_member_stock_returns_only_authenticated_member_stock(): void
    {
        ...
        // Stock for authenticated member
        DB::table('member_stock')->insert([
            'member_stock_member_id' => $memberId,
            'member_stock_product_id' => $product1->product_id,
            'member_stock_balance' => 50,
            'member_stock_transfer_in' => 5,
            'member_stock_transfer_out' => 2,
        ]);

        // Stock for another member
        DB::table('member_stock')->insert([
            'member_stock_member_id' => 999, // Another member
            'member_stock_product_id' => $product2->product_id,
            'member_stock_balance' => 100,
        ]);

        $this->getJson('/api/v1/member/inventory/stocks')
            ->assertOk()
            ->assertJsonPath('success', true)
            ->assertJsonCount(1, 'data.results')
            ->assertJsonPath('data.results.0.product.id', $product1->product_id)
            ->assertJsonPath('data.results.0.balance', 50)
            ->assertJsonPath('data.results.0.transfer_in', 5)
            ->assertJsonPath('data.results.0.transfer_out', 2);
    }
